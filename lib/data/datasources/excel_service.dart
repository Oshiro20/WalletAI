...
class ExcelService {
  ...
  Future<File> exportTransactions() async {
    ...
    for (var t in transactions) {
      ...
      final rowData = [
        ...
        TextCellValue(
          t.type == 'expense'
              ? 'Gasto'
              : (t.type == 'income' ? 'Ingreso' : 'Transferencia'),
        ),
    ...
    final path =
        '${directory.path}/transacciones_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.xlsx';
  ...
  Future<int> importTransactions(File file) async {
    ...
    for (var table in excel.tables.keys) {
      ...
      int idxSubcategory =
          colMap['Subcategoría'] ?? colMap['Subcategorías'] ?? 4;
      int idxNote =
          colMap['Nota'] ?? (isLegacyFormat ? 4 : 6); // Legacy: E=4, Stnd: G=6
      int idxAmount =
          colMap['Importe'] ?? colMap['Monto'] ?? 2; // Legacy: I, Stnd: C=2
      int idxType =
          colMap['Ingreso/Gasto'] ??
          colMap['Tipo'] ??
          1; // Legacy: G, Stnd: B=1
      ...
      for (int i = 1; i < sheet.maxRows; i++) {
        ...
        try {
          ...
          var amountVal = (idxAmount < row.length)
              ? row[idxAmount]?.value
              : null;
          var categoryVal = (idxCategory < row.length)
              ? row[idxCategory]?.value
              : null;
          var subcategoryVal = (idxSubcategory < row.length)
              ? row[idxSubcategory]?.value
              : null;
          var accountVal = (idxAccount < row.length)
              ? row[idxAccount]?.value
              : null;
          ...
          if (accName.isNotEmpty) {
            ...
            try {
              foundAcc = accounts.firstWhere(
                (a) =>
                    _normalize(a.name) == _normalize(accName) ||
                    _normalize(a.name).contains(_normalize(accName)),
              );
            } catch (_) {}
            ...
            if (foundAcc != null) {
              ...
              if (lowerName.contains('efectivo') ||
                ...
                accType = 'cash';
              } else if (lowerName.contains('ahorro') ||
                  lowerName.contains('saving')) {
                accType = 'savings';
              } else if (lowerName.contains('tarjeta') ||
                  lowerName.contains('card') ||
                  lowerName.contains('credito')) {
                accType = 'credit_card';
              } else if (lowerName.contains('yape') ||
                  lowerName.contains('plin') ||
                  lowerName.contains('wallet')) {
                accType = 'wallet';
              } else if (lowerName.contains('inversion') ||
          ...
          if (type != 'transfer' && categoryVal != null) {
            ...
            if (catName.isNotEmpty) {
              ...
              bool isAccountName = accounts.any(
                (a) => _normalize(a.name) == _normalize(catName),
              );
              ...
              if (isAccountName) {
                ...
                debugPrint(
                  'Skipping Category creation for "$catName" as it matches an Account name.',
                );
              } else {
                ...
                if (foundCat == null) {
                  try {
                    foundCat = categories.firstWhere(
                      (c) =>
                          _normalize(c.name).contains(normCatName) &&
                          c.type == type,
                    );
                  } catch (_) {}
                }
              ...
              try {
                foundSub = subcategories.firstWhere(
                  (s) =>
                      ...
                      (_normalize(s.name) == normSubName ||
                          _normalize(s.name).contains(normSubName)),
                );
              } catch (_) {}
  ...
  Future<void> _recalculateAccountBalances(
    dynamic accountsDao,
    dynamic transactionsDao,
  ) async {
    final allAccounts = await accountsDao.getAllAccounts();
    for (var acc in allAccounts) {
      final transactions = await transactionsDao.getTransactionsByAccount(
        acc.id,
      );
