...
const Map<String, List<String>> _categoryKeywords = {
  ...
  'cat_antojos': ['antojo', 'capricho', 'impulso', 'golosina', 'dulce'],
...
class TransactionParserService {
  ParsedTransactionData parse(
    ...
    for (final trigger in accountTriggers) {
      ...
      if (triggerIdx != -1) {
        final afterTrigger = lowerText.substring(
          triggerIdx + normalizedTrigger.length,
        );
    ...
    for (final entry in _subcategoryKeywords.entries) {
      ...
      for (final keyword in sorted) {
        if (lowerText.contains(_normalize(keyword))) {
          subcategoryId = subId;
          final matchedSub = subcategories
              .where((s) => s.id == subId)
              .firstOrNull;
          if (matchedSub != null) {
