...
class NotificationService {
  ...
  Future<void> notifyBudgetWarning({
    ...
    await _show(
      ...
      body: '$categoryName: S/ ${spent.toStringAsFixed(2)} de S/ ${limit.toStringAsFixed(2)}',
  ...
  Future<void> notifyBudgetExceeded({
    ...
    await _show(
      ...
      body: 'Gastaste S/ ${spent.toStringAsFixed(2)} (límite: S/ ${limit.toStringAsFixed(2)})',
      channel: 'budget_alerts',
      channelName: 'Alertas de Presupuesto',
    );
  }
  ...
  Future<void> notifyHighExpense({
    ...
    await _show(
      ...
      body: '"$description" (S/ ${amount.toStringAsFixed(2)}) es ${(amount / dailyAverage).toStringAsFixed(1)}x tu promedio diario.',
  ...
  Future<void> notifyMonthlySummary({
    ...
    await _show(
      ...
      body: 'Ingresos: S/ ${totalIncome.toStringAsFixed(2)} | Gastos: S/ ${totalExpense.toStringAsFixed(2)}',
  ...
  Future<void> checkBudgetAlerts({
    ...
    for (final budget in budgets) {
      final expensesByCategory = await transactionsDao
      ...
      if (pct >= 1.0) {
        await notifyBudgetExceeded(
            categoryName: catName, spent: spent, limit: budget.amount);
      } else if (pct >= 0.8) {
  ...
  Future<void> checkCreditCardDueDates(List<Account> accounts) async {
    ...
    for (final account in accounts) {
      if (account.type != 'credit_card' || account.paymentDueDay == null) continue;
