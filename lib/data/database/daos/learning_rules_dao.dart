...
class LearningRulesDao extends DatabaseAccessor<AppDatabase> with _$LearningRulesDaoMixin {
  ...
  Future<LearningRule?> getRuleForProduct(String productName) {
    return (select(learningRules)..where((t) => t.productName.equals(productName))).getSingleOrNull();
  }
  ...
  Future<void> saveRule(String productName, String categoryId) async {
    ...
    if (existingRule != null) {
       ...
       int newUsageCount = existingRule.categoryId == categoryId ? existingRule.usageCount + 1 : 1;
  ...
  Future<List<LearningRule>> getAllRules() {
    return (select(learningRules)..orderBy([(t) => OrderingTerm.desc(t.lastUsed)])).get();
  }

  /// Eliminar una regla
  Future<void> deleteRule(String productName) {
    return (delete(learningRules)..where((t) => t.productName.equals(productName))).go();
  }
}
