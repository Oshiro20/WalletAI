...
class _CategoryDetailsScreenState extends ConsumerState<CategoryDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final transactionsAsync = ref.watch(
      categoricalTransactionsProvider(
        TransactionFilters(
          startDate: startDate,
          endDate: endDate,
    ...
    final subcategoryBreakdownAsync = ref.watch(
      analyticsSubcategoriesProvider(category.id),
    );
    final allSubcategories =
        ref.watch(allSubcategoriesStreamProvider).asData?.value ?? [];
                ...
                children: [
                  ...
                  subcategoryBreakdownAsync.when(
                    data: (dataMap) {
                      ...
                      final total = dataMap.values.fold(
                        0.0,
                        (sum, val) => sum + val,
                      );
            ...
            transactionsAsync.when(
              data: (transactions) {
                final filteredTransactions = _selectedSubcategoryId == null
                    ...
                    : transactions
                          .where(
                            (t) => t.subcategoryId == _selectedSubcategoryId,
                          )
                          .toList();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (transaction.subcategoryId != null) {
      final sub = allSubcategories
          .where((s) => s.id == transaction.subcategoryId)
          .firstOrNull;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  [
                    if (transaction.productName?.isNotEmpty == true &&
                        transaction.description?.isNotEmpty == true)
                      transaction.description!,
