...
void main() async {
  final filePath = r'D:\Proyectos_Flutter\Aplicativo_Gastos\migration_data\Registro Contable_17-2-2026.xlsx';
  ...
  for (var table in excel.tables.keys) {
    ...
    int idxSubcategory = colMap['SubcategorÃ­a'] ?? colMap['SubcategorÃ­as'] ?? 4;
    ...
    print('Indices: Date=$idxDate, Acc=$idxAccount, Cat=$idxCategory, Sub=$idxSubcategory, Note=$idxNote, Amt=$idxAmount, Type=$idxType');
             ...
             if (dStr.isNotEmpty) {
                 try {
                    ...
                    if (dStr.contains('/')) {
                       ...
                       if (parts.length == 3) {
                          var d = DateTime(int.parse(parts[2]), int.parse(parts[1]), int.parse(parts[0]));
                          print('  Parsed Date: $d');
                       }
                    }
                 } catch (e) {
